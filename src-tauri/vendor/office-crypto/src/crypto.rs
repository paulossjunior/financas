...
/// Hash `data` with the algorithm named in the agile EncryptionInfo XML.
fn hash_digest(algo: &str, data: &[u8]) -> Result<Vec<u8>, DecryptError> {
    match algo {
...
/// AES-CBC decrypt (NoPadding, block-to-block). AES variant chosen by key length.
fn aes_cbc_decrypt(
    key: &[u8],
    iv: &[u8],
    input: &[u8],
...
impl AgileEncryptionInfo {
    ...
    pub fn decrypt(
        ...
        let algo = self.key_data_hash_algorithm.as_str();

        while block_start < (total_size - SEGMENT_LENGTH) {
            let iv = hash_digest(algo, &[key_data_salt, &block_index.to_le_bytes()].concat())?;
            let iv = &iv[..16];
            aes_cbc_decrypt(
                key,
                iv,
        ...
        // last block, possibly < 4096 bytes
        ...
        aes_cbc_decrypt(key, iv, ciphertext, &mut plaintext)?;
    ...
    // this function is expensive: it runs `spin_count` iterations of the chosen hash
    fn iterated_hash_from_password(&self, password: &str) -> Result<Vec<u8>, DecryptError> {
        ...
        let algo = self.password_hash_algorithm.as_str();

        let mut h = hash_digest(algo, &salted)?;
        for i in 0u32..self.spin_count {
            h = hash_digest(algo, &[&i.to_le_bytes()[..], h.as_slice()].concat())?;
        }
        Ok(h)
    }

    fn encryption_key(&self, digest: &[u8], block: &[u8]) -> Result<Vec<u8>, DecryptError> {
        let h = hash_digest(self.password_hash_algorithm.as_str(), &[digest, block].concat())?;
        let key_len = self.password_key_bits as usize / 8;
        validate!(h.len() >= key_len, InvalidStructure)?;
        Ok(h[..key_len].to_owned())
    }

    fn decrypt_aes_cbc(&self, key: &[u8]) -> Result<Vec<u8>, DecryptError> {
        let ciphertext = &self.encrypted_key_value;
        validate!(!ciphertext.is_empty() && ciphertext.len() % 16 == 0, InvalidStructure)?;
        let mut plaintext = vec![0u8; ciphertext.len()];
        aes_cbc_decrypt(key, &self.password_salt, ciphertext, &mut plaintext)?;
        Ok(plaintext)
    }
}
